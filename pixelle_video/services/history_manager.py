...
class HistoryManager:
    ...
    async def resume_task(self, task_id: str, pixelle_video, progress_callback=None):
        """
        Resume a failed or interrupted standard generation task.

        The original input parameters are reused, while `resume_task_id` tells
        the standard pipeline to load the saved storyboard and skip valid assets.
        """
        metadata = await self.persistence.load_task_metadata(task_id)
        if not metadata:
