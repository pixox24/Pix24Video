    ...
    with st.expander(tr("settings.title"), expanded=not is_configured):
        ...
        with comfyui_col:
            with st.container(border=True):
                ...
                st.markdown("---")
                st.markdown(f"**{tr('settings.comfyui.bizyair_title')}**")
                bizyair_api_key = st.text_input(
                    tr("settings.comfyui.bizyair_api_key"),
                    value=comfyui_config.get("bizyair_api_key", ""),
                    type="password",
                    help=tr("settings.comfyui.bizyair_api_key_help"),
                    key="bizyair_api_key_input"
                )
                st.caption(tr("settings.comfyui.bizyair_hint"))

                st.markdown("---")
                st.markdown(f"**{tr('settings.comfyui.minimax_title')}**")
                minimax_config = comfyui_config.get("tts", {}).get("minimax", {})
                minimax_api_key = st.text_input(
                    tr("settings.comfyui.minimax_api_key"),
                    value=minimax_config.get("api_key", ""),
                    type="password",
                    help=tr("settings.comfyui.minimax_api_key_help"),
                    key="minimax_api_key_input"
                )
                st.caption(tr("settings.comfyui.minimax_hint"))
            ...
            if st.button(tr("btn.save_config"), use_container_width=True, key="save_config_btn"):
                try:
                    ...
                    config_manager.set_comfyui_config(
                        ...
                        bizyair_api_key=bizyair_api_key if bizyair_api_key else None,
                        minimax_api_key=minimax_api_key if minimax_api_key else ""
                    )
